...
class Vulnerability_Assesment_Tool():
    ...
    ######################################
    #CVE LOOKUP BLOCK:
    async def CVE_Lookup(self, banner):  #Tool that looks upon the documentations
        url = f"https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch={banner}"  #API and automatic search
        async with aiohttp.ClientSession() as session: #Creating the session
            try: 
                async with session.get(url, timeout=ClientTimeout(total=10)) as r: # get requestp tp the api
                    data = await r.json() # Listing all the data in a dictonary
                    vulnerabilities = data["vulnerabilities"] # taking only the vulnerabilities
                    for v in vulnerabilities: # going along each vulnerability
                        ...
                        score = v["cve"]["metrics"]["cvssMetricV31"][0]["cvssData"]["baseScore"] #variable takes all score information
                        print(f"[CVE] {cve_id} | Score: {score} | {description}") # Meaning full output         
            except Exception as e:
                print(f"[CVE ERROR] {e}")
    ######################################       
